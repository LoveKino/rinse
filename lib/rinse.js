...
var ObjectScene = (function() {
	...
	var diffObject = function(pointer, standerdMap, path, ignore) {
		var pointerProperties = Object.getOwnPropertyNames(pointer);
		var standerdMapProperties = Object.getOwnPropertyNames(standerdMap);

		for (var i = 0; i < pointerProperties.length; i++) {
			var name = pointerProperties[i];
		...
		for (var i = 0; i < standerdMapProperties.length; i++) {
			var name = standerdMapProperties[i];
	...
	return {
		create: function(obj, conf) {
			conf = conf || {};
			var maxDepth = conf.maxDepth;
			var ignore = conf.ignore;
			var obj = obj;
			var tree = captureobject(obj, maxDepth);
